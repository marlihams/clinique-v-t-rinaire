...
<html>
	...
	<body>
			...
			if ($tel !=''){
				...
				$vSql ="SELECT nom, prenom FROM vveterinaire WHERE id='$id';";
				$vQuery=pg_query($vConn, $vSql);
				while ($vResult = pg_fetch_array($vQuery, null, PGSQL_ASSOC)) {
					$nom="$vResult[nom]";
					$prenom="$vResult[prenom]";
				...
				if($nom!="" && $prenom!="") {
					$veto=True;
					echo"<div id='titre'>Bienvenu $prenom $nom</div>";
				...
				echo "<form id='connection' method='post' action='principale.php' >";
